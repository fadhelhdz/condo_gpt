    ...
    def _run(self, address):
        try:
            ...
            if geocode_result:
                ...
                print(address)
                print(location)
                print("--- GEOCODING TOO END ---")
                return f'The coordinates for the address {address} are {location["lat"]}, {location["lng"]}.'
            return "Unable to find the coordinates for the specific address."
        except Exception as e:
            return f"An error occured while fetching coordinates: {str(e)}"

class DirectionsInput(BaseModel):
    origin: str = Field(..., description="The starting point address or coordinates")
    destination: str = Field(..., description="The destination point or coordinates")

class DirectionsTool(BaseTool):
    name = "google_maps_directions"
    description = "Useful for finding travel distances and directions between two locations"
    args_schema = DirectionsInput
    ...
    def _run(self, origin, destination):
        try:
            ...
            return f"An error occured while fetching directions: {str(e)}"
